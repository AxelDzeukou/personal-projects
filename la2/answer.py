...
def basic_als_recommender(filename, seed):
    ...
    *ALS.setSeed()*). The following parameters must be used in the ALS
    ...
    als = ALS(maxIter=5, rank=70, regParam=0.01, seed=seed, userCol="userId", itemCol="movieId", ratingCol="rating",
              coldStartStrategy="drop")
...
def global_average(filename, seed):
    ...
    return float(training.agg({'rating': 'sum'}).collect()[0]['sum(rating)']/training.count())
...
def global_average_recommender(filename, seed):
    ...
    globalaverage=float(training.agg({'rating': 'sum'}).collect()[0]['sum(rating)']/training.count())

    evaluator = RegressionEvaluator(metricName="rmse", labelCol="rating",
                                    predictionCol="prediction")
    rmse = evaluator.evaluate(test.withColumn("prediction",lit(globalaverage)))
    return rmse
...
def means_and_interaction(filename, seed, n):
    ...
    useraveragedf = training.groupBy("userId").avg("rating").withColumnRenamed("avg(rating)","user_mean")
    itemaveragedf=training.groupBy("movieId").avg("rating").withColumnRenamed("avg(rating)","item_mean")

    return training.join(useraveragedf,"userId").join(itemaveragedf, "movieId").withColumn(
        "user_item_interaction",col("rating")-(col("user_mean")+col("item_mean")-globalaverage)).sort(['userId', 'movieId']).take(n)
...
def als_with_bias_recommender(filename, seed):
    ...
    training= training.join(useraveragedf, "userId").join(itemaveragedf, "movieId").withColumn(
        "user_item_interaction", col("rating") - (col("user_mean") + col("item_mean") - globalaverage))

    test=test.join(useraveragedf, "userId").join(itemaveragedf, "movieId").withColumn(
        "user_item_interaction", col("rating") - (col("user_mean") + col("item_mean") - globalaverage))

    # Build the recommendation model using ALS on the training data
    # Note we set cold start strategy to 'drop' to ensure we don't get NaN evaluation metrics
    als = ALS(maxIter=5, rank=70, regParam=0.01, seed=seed, userCol="userId", itemCol="movieId", ratingCol="user_item_interaction",
              coldStartStrategy="drop")
    model = als.fit(training)

    # Evaluate the model by computing the RMSE on the test data
    #i=predicted uii
    predictions = model.transform(test).withColumnRenamed("prediction", "uiiPrediction").withColumn(
        "prediction", col("uiiPrediction")+col("user_mean") + col("item_mean") - globalaverage)
