...
#         winner_i = response.css(
#             'div.b-fight-details__person i.b-fight-details__person-status.b-fight-details__person-status_style_green'
#         )
#         winner = "Unknown"
#         if winner_i:
#             winner_container = winner_i.xpath('ancestor::div[contains(@class,"b-fight-details__person")]').get()
#             if winner_container:
#                 winner_tag = response.xpath('string(' + winner_container + ')').get()
#                 if winner_tag:
#                     winner = winner_tag.strip()
#         else:
#             gray_status = response.css(
#                 'div.b-fight-details__person i.b-fight-details__person-status.b-fight-details__person-status_style_gray::text'
#             ).get()
#             if gray_status and gray_status.strip() in ['NC', 'D']:
#                 winner = gray_status.strip()
#         self.fight_data['Winner'].append(winner)

#         # Extract stats safely
#         fight_stats_tr_list = response.css('tr')
#         if len(fight_stats_tr_list) > 1:
#             tds = fight_stats_tr_list[1].css('td')
...
#                 'fighter_A_CTRL','fighter_B_CTRL'
#             ]):
#                 # split A/B stats based on index pattern
#                 td_idx = idx // 2
#                 part = idx % 2
#                 try:
#                     value = tds[td_idx].css('p')[part].css('::text').get(default='').strip()
#                 except IndexError:
#                     value = None
#                 self.fight_data[key].append(value)
#         else:
#             # no stats table
